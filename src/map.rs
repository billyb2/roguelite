...
use crate::{
    draw::Drawable,
    math::{aabb_collision, AsAABB, AxisAlignedBoundingBox},
    monsters::{Monster, SmallRat},
    player::Player,
};
    ...
    pub fn new(_floor_num: usize, textures: &HashMap<String, Texture2D>) -> Self {
        ...
        for _ in 0..100_000 {
            ...
            if !rooms.iter().any(|room: &Room| {
                // Don't let rooms be just one room apart, since moving through doors fro both rooms is annoying to the player
                // Also don't let rooms collide w each other
                ...
                (room.bottom_right + MIN_DISTANCE_BETWEEN_ROOMS)
                    ...
                    .all()
                    && (bottom_right + MIN_DISTANCE_BETWEEN_ROOMS)
        ...
        positions
            ...
            .for_each(|(p, change)| {
                ...
                if new_pos.x < 0
                    ...
                    || new_pos.y < 0
                    || new_pos.y >= MAP_HEIGHT_TILES.try_into().unwrap()
                {
                    *p = None;
                } else {
                    *p = Some(new_pos);
